...
class SnkModelEvent extends JModelAdmin {
	...
	function getItem($pk = null){
		...
		$data = array(
			...
			'Created_at' => strftime('%Y-%m-%d %H:%M:%S',$event['Created_At']),
			'Modified_by' => $event['Last_Modified_By'],
			'Modified_at' => strftime('%Y-%m-%d %H:%M:%S',$event['Last_Modified_At']),
			'SSID' => $this->ssid,
			'Title' => $event['Title'],
			'Organizer' => $event['Organizer'],
			'Target_Group' => $event['TargetGroup'],
			'Start_Date' => strftime('%Y-%m-%d',$event['Start']), 
			'Start_Time' => strftime('%H:%M',$event['Start']), 
			'End_Date' => strftime('%Y-%m-%d',$event['End']), 
