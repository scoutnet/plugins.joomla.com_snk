...
if [ ! -e build/$name-$version-final.zip ]; then
	...
	for update_version in $versions; do
		for joomla_version in '1.6' '1.7' '2.5'; do
			for client_id in '' '<client_id>1<\/client_id>'; do

				xml=$(cat update_template.xml | sed "s/###NAME###/$name/g" | sed "s/###VERSION###/$update_version/g" | sed "s/###CLIENT_ID###/$client_id/g" | sed "s/###JOOMLA_VERSION###/$joomla_version/g")
